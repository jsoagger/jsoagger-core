pipeline {
   ...
   stages {
        ...
        stage('Build') {
        	when {
	            branch 'master'
	            not{ equals expected: "jenkins", actual: "${AUTHOR}" }
	        }
	        
            steps {
            	ansiColor('xterm') {
            		sh "mvn -version"
                	sh "mvn -Dmaven.test.failure.ignore=false -DskipTests=false -Dmaven.javadoc.skip=true clean install"
                }
            }
        }
        		...
        		timeout(time: 5, unit: 'MINUTES'){
        			script {
	                    def perfomRelease = input(
 							id: 'perfomRelease', message: 'Do you want to release?', ok:'Yes, i release this build' 
						)
                	}	
        		}
        	}
